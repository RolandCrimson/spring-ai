package com.adacho.ch13_agent.agent;
...
import com.adacho.ch13_agent.advisor.CityValidationAdvisor;
 ...
 * 2. 사용자가 '오늘', '지금', '현재'를 언급하면 getCurrentWeather를 사용하세요.
 * 3. 그 외의 경우에는 질문의 의도에 맞게 적절한 Tool을 선택하거나, 예보가 불가능한 시점에 대해서는 일반적인 기후 특성을 안내하세요.
 * 4. 사용자가 도시를 명시하지 않고 후속 질문(예: "그럼 내일은?", "이번 주는?")을 하면, 이전 대화에서 마지막으로 언급된 도시를
 * 기준으로 답변하세요.
 * 
 * ## 제공할 정보
 * - 평균 기온 (최저/최고)
 ...
 * 
 * @Tool(description =
 * "현재 날짜와 시간을 조회합니다. 사용자가 특정 월이나 시기에 대해 물을 때 먼저 이 도구를 사용하여 현재 날짜를 확인하세요.")
 * public String getCurrentDateTime() {
 * LocalDate now = LocalDate.now();
 * String formattedDate =
 * now.format(DateTimeFormatter.ofPattern("yyyy년 MM월 dd일 E요일", Locale.KOREAN));
